...
-\\
Slides at\\
\hspace*{0.333em}\url{https://github.com/BITSS/CEGA2021}}
\date{CEGA, March 2021}
...
\begin{Highlighting}[]
\SpecialCharTok{{-}{-}{-}}
\NormalTok{title}\SpecialCharTok{:} \StringTok{"Sample Paper"}
\NormalTok{author}\SpecialCharTok{:} \StringTok{"Fernando Hoces de la Guardia"}
...
\begin{quote}
There are 5 in this room.
\end{quote}

\begin{quote}
Is it something like \(\frac{1}{365} \times 5=\) 0.014?
\end{quote}
\end{frame}
...
\NormalTok{\textbackslash{}begin\{align\} }
 \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ \textbackslash{}bar }\FunctionTok{p}\NormalTok{(n) }\SpecialCharTok{\&}\ErrorTok{=} \DecValTok{1}\NormalTok{ \textbackslash{}times \textbackslash{}}\FunctionTok{left}\NormalTok{(}\DecValTok{1}\SpecialCharTok{{-}}\NormalTok{\textbackslash{}frac\{}\DecValTok{1}\NormalTok{\}\{}\DecValTok{365}\NormalTok{\}}
\NormalTok{                                 \textbackslash{}right) }
\NormalTok{ \textbackslash{}times ...}
\NormalTok{ A lot of equations using LateX syntax}\SpecialCharTok{!}
...
1 - Simulate 10,000 rooms with \(n = 5\) random birthdays, and store the
results in matrix where each row represents a room.\\
2 - For each room (row) compute the number of unique birthdays.\\
3 - Compute the average number of times a room has 5 unique birthdays,
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{birthday.prob }\OtherTok{=} \ControlFlowTok{function}\NormalTok{(n.pers, n.sims) \{}
  \CommentTok{\# simulate birthdays}
\NormalTok{  birthdays }\OtherTok{=} \FunctionTok{matrix}\NormalTok{(}\FunctionTok{round}\NormalTok{(}\FunctionTok{runif}\NormalTok{(n.pers }\SpecialCharTok{*}\NormalTok{ n.sims, }
                                 \DecValTok{1}\NormalTok{, }\DecValTok{365}\NormalTok{)), }
                      \AttributeTok{nrow =}\NormalTok{ n.sims, }\AttributeTok{ncol =}\NormalTok{ n.pers)}
...
\NormalTok{  all.different }\OtherTok{=} \DecValTok{1} \SpecialCharTok{*}\NormalTok{ (unique.birthdays}\SpecialCharTok{==}\NormalTok{n.pers)}
...
\NormalTok{\}}
\NormalTok{n.pers.param }\OtherTok{=}\NormalTok{ n.pers; n.sims.param }\OtherTok{=} \FloatTok{1e4}
\FunctionTok{birthday.prob}\NormalTok{(n.pers.param,n.sims.param)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \(\frac{1}{365} \times N =\) 0.014
\item
  However the true probability is of \(p(n= 5) = 0.027\)
\item
  And the simulated probability is of 0.0274
\end{itemize}
\end{frame}
